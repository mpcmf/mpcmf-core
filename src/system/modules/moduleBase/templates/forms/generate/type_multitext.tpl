<script>
        ...
        $(document).ready(function () {
            let body = $('body');
            body.on('click', '.filterParams.{{$fieldName}} .add', function () {
                let index = parseInt($('.filterParams.{{$fieldName}} .form-control').last().attr('index')) + 1;
                $(this).parent().parent()
            ...
            body.on('click', '.filterParams.{{$fieldName}} .remove', function () {
                let filterParamsInputs = $(this).parent().parent().children();
                let count = filterParamsInputs.length;
                if (count > 1) {
                    $(this).parent().remove();
                } else {
...
<div class="filterParams {{$fieldName}}">
